...
// Advent of Code 2016. Day Four.

$instructions = explode(PHP_EOL, 'gbc-frperg-pubpbyngr-znantrzrag-377[rgbnp]
...
$tsid = $reqid = 0;
$reqroom = 'northpole object storage';

foreach ($instructions as $instruction) {
    $cs = substr($instruction, -6, -1);
    $sid = substr($instruction, -10, -7);
    $newins = str_replace('-', ' ', substr($instruction, 0, -11));
    $newins2 = str_replace(' ', '', $newins);
    $l = [];
    $t = [];
    for ($j = 0; $j <= strlen($newins2) - 1; $j++) {
        $letter = substr($newins2, $j, 1);
        if (!isset($l[$letter])) { $l[$letter] = $letter; }
        !isset($t[$letter]) ? $t[$letter] = 1 : $t[$letter]++;
    }
    array_multisort($t, SORT_DESC, SORT_NUMERIC, $l);
    $l = array_values($l);
    ...
    if ($out == $cs) {
        $tsid += $sid;
        if (rotate($newins, $sid) === $reqroom) {
            $reqid = $sid;
        }
    }
}

echo 'Sum of sector IDs: ' . number_format($tsid) . '.' . PHP_EOL;
echo "Sector ID of '$reqroom': " . number_format($reqid) . '.' . PHP_EOL;

// Sum of sector IDs: 409,147.
// Sector ID of 'northpole object storage': 991.
