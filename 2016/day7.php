...
// Advent of Code 2016. Day Seven.

$instructions = explode(PHP_EOL, 'xdsqxnovprgovwzkus[fmadbfsbqwzzrzrgdg]aeqornszgvbizdm
...
foreach ($instructions as $ikey => $i) {
    preg_match_all('/\[.+?\]/', $i, $bmatch);
    preg_match_all('/^.+?\[|\].+?\[|\].+?$/', $i, $nbmatch);

    foreach ($bmatch[0] as $mkey => $m) {
	    $bmatch[0][$mkey] = str_replace(['[', ']'], '', $m);
	}
    foreach ($nbmatch[0] as $mkey => $m) {
	    $nbmatch[0][$mkey] = str_replace(['[', ']'], '', $m);
	}

    foreach ($bmatch[0] as $mkey => $m) {
        for ($j = 0; $j <= strlen($m) - 4; $j++) {
            if (substr($m, $j, 1) == substr($m, $j + 3, 1) && substr($m, $j + 1, 1) == substr($m, $j + 2, 1) && substr($m, $j, 1) != substr($m, $j + 1, 1)) {
...
echo 'IPs supporting TLS (Part One): ' . count($instructions) . PHP_EOL;

foreach ($instructions2 as $ikey => $i) {
    preg_match_all('/^.+?\[|\].+?\[|\].+?$/', $i, $nbmatch);
    preg_match_all('/\[.+?\]/', $i, $bmatch);
    ...
    $aba = [];
    foreach ($nbmatch[0] as $mkey => $m) {
        for ($j = 0; $j <= strlen($m) - 3; $j++) {
            if (substr($m, $j, 1) == substr($m, $j + 2, 1) && substr($m, $j, 1) != substr($m, $j + 1, 1)) {
                $aba[] = substr($m, $j, 3);
            }
        }
    }
    ...
    foreach ($aba as $k => $v) {
        $aba[$k] = substr($v, 1, 2) . substr($v, 1, 1);
    }
    ...
    if (!$mtch) {
        unset($instructions2[$ikey]);
    }
}

echo 'IPs supporting SSL (Part Two): ' . count($instructions2) . PHP_EOL;

// IPs supporting TLS (Part One): 115
// IPs supporting SSL (Part Two): 231
