...
// Advent of Code 2017. Day Two. http://adventofcode.com/2017/day/2

$out1 = $out2 = 0;

$instructions = '278	1689	250	1512	1792	1974	175	1639	235	1635	1690	1947	810	224	928	859
...
foreach ($split_lineend as $line) {
    ...
    $out1 += max($split_line) - min($split_line);
    for ($j = 0, $c = count($split_line); $j <= $c - 1; $j++) {
        for ($k = $j + 1; $k <= $c - 1; $k++) {
            $sum1 = (int) $split_line[$j] / (int) $split_line[$k];
            if (is_integer($sum1)) {
                $out2 += $sum1;
                break 2;
            }
            $sum2 = (int) $split_line[$k] / (int) $split_line[$j];
            if (is_integer($sum2)) {
                $out2 += $sum2;
                break 2;
            }
