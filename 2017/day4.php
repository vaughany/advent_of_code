...
// Advent of Code 2017. Day Four. http://adventofcode.com/2017/day/4

$out1 = $out2 = 0;

$instructions = 'bdwdjjo avricm cjbmj ran lmfsom ivsof
...
foreach ($split_lineend as $line) {
    ...
    foreach ($split_line as $word) {
        ...
        for ($j = 0; $j <= strlen($word) -1; $j++ ) {
            if (!isset($newword[substr($word, $j, 1)])) {
                $newword[substr($word, $j, 1)] = 1;
            } else {
                $newword[substr($word, $j, 1)]++;
            }
        }
        ksort($newword);
        if (empty($words)) {
